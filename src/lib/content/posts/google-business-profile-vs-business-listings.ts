const markdown = `GBP alone is not a full listings program. Apple Maps, Bing Places, Facebook, Yelp, aggregators, in-car navigation, and the directory network still carry US customers when Google does not control the session. Managing only GBP is necessary and not sufficient.
